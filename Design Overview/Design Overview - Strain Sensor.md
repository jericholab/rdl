...
- Note however that the TAMURA sensor is not installed on the PCB. You will notice on the strain sensor that the TAMURA space is present, but the sensor absent. Therefore, the same PCB is used for two separate Jericho products:
...
- The NAU7802 has two functions: it provides a regulated excitation voltage to the Wheatstone bridge and it measures the response signal.
...
- The NAU7802 chip is equipped with decoupling capacitors at the power supply (0.1uF, 10uF and 22uF). While other RDL boards use only 0.1uF and 10uF, the NAU7802 uses a supplementary 22uF for increased stability.
- In theory, the NAU7802 does not require a precise supply voltage (2.7-5.5V) since it has an internal voltage regulator. However, the input decoupling capacitors, by stabilizing the input voltage, ease the work of the voltage regulator and improve performance by removing noise and other voltage fluctuations.
- Multiple users of the similar Sparkfun NAU7802 board reported high sensitivity of readings to voltage supply, hence, a voltage regulator circuit was added.
...
- The Wheatstone bridge operates at the default 3.3V **STILL TRUE? NOT 5V NOWADAYS**. It is within the allowable excitation voltage range of the TAL220 (3-10V) and it maximize the measurement range by minimizing the sensitivity (V/N).
...
- The native code of the RDL (revE4 and beyond) can operate the strain sensor. The number of allowable strain sensors is only limited by the number of I2C channels.
...
- For the RDL, internal calibration is done automatically by a function from the NAU7802 library (calibrateAFE()). It is called at power up and at a regular interval. The algorithm is still under development for the regular interval.
