...
void simpleVoltFunc(){
      ...
      for (int i=0; i< (numberV); i++) {   
        ...
        pcf1.digitalWrite(i, LOW);  // turn on by sinking current to ground
        pcf2.digitalWrite(i, HIGH);  // turn on by sinking current to ground
        delay(100);
        value=getVoltFunc(readModeVolt);                             // storing voltages 
        Serial.print(value,4);
        spacing1(value,10);
        pcf1.digitalWrite(i, HIGH); //turn off by turning off sinking transistor
        pcf2.digitalWrite(i, LOW);  //turn off by turning off sinking transistor
      }
}
