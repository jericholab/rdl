...
fprintf('Evaluation of measurement uncertainty for a thermistor ...\r\n')
fprintf('... with a given accuracy expressed as a constant fraction of its resistance\r\n')
fprintf('\r\n')
...
A = 1.1009515911e-03 %Steinhart-Hart coefficient A  (uncertainty results not very sensitive to exact coefficients)
...
%Rs = 5000 %ohm %%%%%%% TEMPORARY TEST ASSUMING A DESIGN WITH A SMALLER RS
...
% - - - - - - - - - - - - - - - - - - - - - - 
% PART 5: Measurement uncertainty components as a function of temperature (FULL RANGE)
...
%dT3=0.1; %%%%%%%%% TEMPORARY TEST ASSUMING A PRECISION OF 0.1C for all calibration points
...
%Figure (5): Plotting uncertainty results (0 - 100C RANGE)
subplot(2,3,5); 
plot(M(66:783,1),M(66:783,2),'-')   %only plot the rows included in the 0-100C range
title('Fig.5. Measurement uncertainty components as a function of temperature')
xlabel('Temperature (oC)')
ylabel('Uncertainty (oC))')
grid on;
hold on;
plot(M(66:783,1),M(66:783,3),'-') 
plot(M(66:783,1),M(66:783,4),'s')
plot(M(66:783,1),M(66:783,5),'-o')
legend show
legend('ADC','Rs','R','Total')

%Figure (6): Plotting uncertainty results (FULL RANGE OF TEMPERATURES)
subplot(2,3,6); 
semilogy(M(:,1),M(:,2),'-') 
title('Fig.5. Measurement uncertainty components as a function of temperature')
xlabel('Temperature (oC)')
